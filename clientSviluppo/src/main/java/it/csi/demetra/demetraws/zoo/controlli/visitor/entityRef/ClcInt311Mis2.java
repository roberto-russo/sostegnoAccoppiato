...
public class ClcInt311Mis2 extends Controllo {
	...
	public void preEsecuzione() throws ControlloException, CalcoloException {
		logger.info("INIZIO CALCOLO INTERVENTO 311 MISURA 2");
		
		if(logger.isDebugEnabled())
			logger.debug("CALCOLO INTERVENTO 311 MISURA 2, INIZIO PRE-ESECUZIONE");
			...
			try {
				...
				if (!modelVaccheAmmesse.isEmpty()) {
					...
					if (!resultCtlUba.isErrors()) {
						...
						logger.error("ERRORE CALCOLO INTERVENTO 311 MISURA 2, ERRORE DURANTE IL CALCOLO DELLE UBA MINIME.");
					...
					logger.error("ERRORE CALCOLO INTERVENTO 311 MISURA 2, NESSUNA VACCA PRESENTE, NON E' POSSIBILE CALCOLARE LE UBA MINIME.");
					throw new ControlloException(new Dmt_t_errore(getSessione(), "REF_9903", getInput(),
				...
				logger.error("ERRORE DI ESECUZIONE CALCOLO INTERVENTO 311 MISURA 2");
		...
		if(logger.isDebugEnabled())
			logger.debug("CALCOLO INTERVENTO 311 MISURA 2, FINE PRE-ESECUZIONE");
		 logger.info("I CONTROLLI DI PRE-CALCOLO PER IL CALCOLO INTERVENTO 311 MISURA 2 SONO STATI ESEGUITI CORRETTAMENTE ✔");
	}
	...
	public void esecuzione() throws ControlloException {
		 ...
		 */
		if(logger.isDebugEnabled())
			logger.debug("CALCOLO INTERVENTO 311 MISURA 2, INIZIO ESECUZIONE");
		if (modelVaccheAmmesse != null && !modelVaccheAmmesse.isEmpty()) {
			...
			if (ubaMinimeRaggiunte) {
				...
				logger.error("ERRORE CALCOLO INTERVENTO 311 MISURA 2, NON E' STATO RAGGIUNTO IL NUMERO DELLE UBA MINIME NECESSARIE AL RAGGIUNGIMENTO DEL PREMIO REF02.002");
			...
			logger.error("ERRORE CALCOLO INTERVENTO 311 MISURA 2, NESSUNA VACCA E' STATA AMMESSA A PREMIO REF02002");
		...
		if(logger.isDebugEnabled())
			logger.debug("CALCOLO INTERVENTO 311 MISURA 2, FINE ESECUZIONE");
	}
	...
	public void postEsecuzione() throws ControlloException {
		...
		if(logger.isDebugEnabled())
			logger.debug("CALCOLO INTERVENTO 311 MISURA 2, INIZIO POST-ESECUZIONE");
		...
		if(logger.isDebugEnabled())
			logger.debug("CALCOLO INTERVENTO 311 MISURA 2, FINE POST-ESECUZIONE");
		logger.info("FINE ESECUZIONE CALCOLO INTERVENTO 311 MISURA 2");
	}
