...
public class ref03 {
    ...
    public void inizializzazione(Dmt_t_sessione sessione, Rpu_V_pratica_zoote azienda) {
        logger.info("INIZIO CALCOLO REF03 AMMISIBILITA' PREMIO E SANZIONI");
    ...
    public void esecuzione() throws CalcoloException {
        if (logger.isDebugEnabled())
            logger.debug("CALCOLO REF03, INIZIO ESECUZIONE");
        ...
        try {
            ...
            logger.error("ERRORE CALCOLO REF03, ERRORE DURANTE IL RECUPERO DEGLI IMPORTI UNITARI");
        ...
        for (String cp : capiPerPremio.keySet()) {
            ...
            try {
                logger.info(this.azienda.getAnnoCampagna());
                logger.info(cp);
                ...
                logger.info("CHECK IMPORTO NULL -> ");
                logger.info(o);
                this.importoUnit = new BigDecimal(o
                        .getImportoUnitario());
            } catch (Exception e) {
                logger.error("ERRORE CALCOLO REF03, ERRORE DURANTE IL RECUPERO DELL'IMPORTO UNITARIO, CP -> " + cp);
                e.printStackTrace();
            ...
            if (cp.equals("320") && capiPagabili.compareTo(BigDecimal.ZERO) > 0) {
                try {
                    ...
                    logger.error("ERRORE CALCOLO REF03, ERRORE DURANTE IL CALCOLO DELL'IMPORTO PAGATO AL LORDO DELLA DECURTAZIONE");
                    e.printStackTrace();
            ...
            if (giorniRitardo != null && !giorniRitardo.equals(new Integer(0))) {
                try {
                    ...
                    logger.error("ERRORE CALCOLO REF03, ERRORE DURANTE IL CALCOLO DELL'IMPORTO PAGATO AL NETTO DELLA DECURTAZIONE");
        ...
        if (logger.isDebugEnabled())
            logger.debug("CALCOLO REF03, FINE ESECUZIONE");
        logger.info("FINE CALCOLO REF03 AMMISIBILITA' AL PREMIO E SANZIONI");
    }
