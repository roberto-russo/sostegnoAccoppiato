...
public interface Dmt_t_tws_bdn_du_capi_bovini_repository extends CrudRepository<Dmt_t_Tws_bdn_du_capi_bovini, Long> {
    ...
    @Query(value = "SELECT * " +
            "FROM DMT_T_TWS_BDN_DU_CAPI_BOV DMT_BOV " +
            "WHERE dmt_bov.id_sessione = :idSessione " +
            "    and dmt_bov.cuaa = :cuaa " +
            "    and dmt_bov.codice_premio = :codiceIntervento " +
            "    and dmt_bov.id_allevamento in ( " +
            "        select allev_id " +
            "        from dmt_t_anagrafica_allev allev " +
            "        where ( " +
            "                allev.COD_FISCALE_PROP = dmt_bov.cuaa " +
            "                or COD_FISCALE_DETEN = dmt_bov.cuaa " +
            "            ) " +
            "            and allev.id_sessione = dmt_bov.id_sessione " +
            "            and DT_FINE_ATTIVITA is null " +
            "    )", nativeQuery = true)
    ...
    @Query(
            value = "SELECT * FROM DMT_T_TWS_BDN_DU_CAPI_BOV where id_sessione = :idSessione and CUAA = :cuaa and codice_premio IN ('316','317','318','319')",
            nativeQuery = true
    )
    List<Dmt_t_Tws_bdn_du_capi_bovini> findM19BySessioneAndCuaa(@Param("idSessione") Long idSessione, @Param("cuaa") String cuaa);
