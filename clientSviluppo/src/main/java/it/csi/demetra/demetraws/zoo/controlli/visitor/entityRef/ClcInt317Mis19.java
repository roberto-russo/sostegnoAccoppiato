 ...
 * di qualità nazionale o regionale e allevati dal richiedente per un periodo
...
public class ClcInt317Mis19 extends Controllo {
	 ...
	 * intervento 317 misura 19. Se l'esecuzione ha esito negativo, allora viene
	...
	public void preEsecuzione() throws ControlloException {
		logger.info("INIZIO CALCOLO INTERVENTO 317 MISURA 19");
		if(logger.isDebugEnabled())
			logger.debug("CALCOLO INTERVENTO 317 MISURA 19, INIZIO PRE-ESECUZIONE");
		...
		try {
			...
			if (ubaMin.isErrors()) {
				logger.error("ERRORE CALCOLO INTERVENTO 317 MISURA 19, ERRORE DURANTE L'ESECUZIONE DEL CONTROLLO DELLE UBA MINIME");
				throw new CalcoloException("errore durante l'esecuzione del controllo delle uba minime");
			}
			else if (!ubaMin.isResult()) {
				logger.error("ERRORE CALCOLO INTERVENTO 317 MISURA 19, ERRORE DURANTE L'ESECUZIONE DEL CALCOLO INTERVENTO 317 MISURA 19");
				throw new ControlloException(new Dmt_t_errore(getSessione(), "ClcInt317Mis19", getInput(),
						"controllo uba minime non rispettato"));
			}

		} catch (CalcoloException e) {
			logger.error("ERRORE CALCOLO INTERVENTO 317 MISURA 19, ERRORE DURANTE L'ESECUZIONE DEI CONTROLLI DI AMMISIBILITA' TRASVERSALI REF99.03");
		...
		if(logger.isDebugEnabled())
			logger.debug("CALCOLO INTERVENTO 317 MISURA 19, FINE PRE-ESECUZIONE");
		logger.info("I CONTROLLI DI PRE-CALCOLO PER IL CALCOLO INTERVENTO 317 MISURA 19 SONO STATI ESEGUITI CORRETTAMENTE ✔");
	...
	public void esecuzione() throws ControlloException{
		...
		if(logger.isDebugEnabled())
			logger.debug("CALCOLO INTERVENTO 317 MISURA 19, INIZIO ESECUZIONE");
		...
		if(this.estrazioneACampione == null || this.estrazioneACampione.isEmpty()) {
			...
			try {
				...
				for (Dmt_t_clsCapoMacellato m : this.modelMacellatoFiltrato) {
					...
					logger.error("ERRORE CALCOLO INTERVENTO 317 MISURA 19, NESSUN CAPO HA SUPERATO IL CONTROLLO PER IL PREMIO");
					...
					logger.error("ERRORE CALCOLO INTERVENTO 317 MISURA 19, ERRORE DURANTE L'ESECUZIONE DEL CALCOLO INTERVENTO 317 MISURA 19 REF02.009");
				new Dmt_t_errore(getSessione(), "ref02_009", getInput(), e.getMessage());
			}catch (NullPointerException e){
				logger.error("ERRORE DURANTE L'ESECUZIONE DEL CALCOLO INTERVENTO 317 MISURA 19, NESSUN CAPO DISPONIBILE");
		...
		if(logger.isDebugEnabled())
			logger.debug("CALCOLO INTERVENTO 317 MISURA 19, FINE ESECUZIONE");
	 ...
	 * 
	 * @throws ControlloException
	 *             eccezione relativa ai controlli di tipo
	 *             {@link ControlloException}
	 */
	@Override
	public void postEsecuzione() throws ControlloException {
		...
		if(logger.isDebugEnabled())
			logger.debug("CALCOLO INTERVENTO 317 MISURA 19, INIZIO POST-ESECUZIONE");
		...
		if(logger.isDebugEnabled())
			logger.debug("CALCOLO 316 MISURA 19, FINE POST-ESECUZIONE");
		 logger.info("FINE ESECUZIONE CALCOLO INTERVENTO 317 MISURA 19");
	}
