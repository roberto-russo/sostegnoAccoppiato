...
public class ClcInt316Mis19 extends Controllo {
	 ...
	 * intervento 316 misura 19. Se l'esecuzione ha esito negativo, allora viene
	...
	public void preEsecuzione() throws ControlloException {
		logger.info("INIZIO CALCOLO INTERVENTO 316 MISURA 19");
		if(logger.isDebugEnabled())
			logger.debug("CALCOLO INTERVENTO 316 MISURA 19, INIZIO PRE-ESECUZIONE");
		...
		try {
			...
			logger.error("ERRORE CALCOLO INTERVENTO 316 MISURA 19, ERRORE DURANTE L'ESECUZIONE DEI CONTROLLI AMMISIBILITA' TRASVERSALI REF99.03");
		...
		if(logger.isDebugEnabled())
			logger.debug("CALCOLO INTERVENTO 316 MISURA 19, FINE PRE-ESECUZIONE");
		logger.info("I CONTROLLI DI PRE-CALCOLO PER IL CALCOLO INTERVENTO 316 MISURA 19 SONO STATI ESEGUITI CORRETTAMENTE ✔");

	}

	/**
	 * nel metodo esecuzione vengono eseguiti i controlli per il calcolo
	 * intervento 316 misura 19. Se i controlli per il suddetto calcolo
	 * risultano essere positivi, allora viene incrementato il contatore di
	 * importo liquidabile e il capo sarà visibile in
	...
	public void esecuzione() throws ControlloException {
		...
		if(logger.isDebugEnabled())
			logger.debug("CALCOLO INTERVENTO 316 MISURA 19, INIZIO ESECUZIONE");
		...
		if (this.estrazioneACampione == null || this.estrazioneACampione.isEmpty()) {
			...
			try {
					...
					if (UtilControlli.flagDuplicatiRichiedenti(duplicatiMacellati, getAzienda().getCuaa(), this.getControlliService())) {
						...
						if (UtilControlli.controlloTempisticheDiRegistrazione(m)) {
							...
							if ((m.getDtUscita() != null && m.getDtInserimentoBdnIngresso() != null) && (UtilControlli
								...
								/*
								 * il capo non è stato allevato per un periodo minimo di
						...
						/*
						 * il capo è stato richiesto in pagamento da più
						 * di un soggetto, il capo non può esserepagato
						 * a meno di una rinuncia da parte di uno dei
						 * richiedenti.
						 */
				...
				if (importoLiquidabile.compareTo(BigDecimal.ZERO) == 0) {
					logger.error("ERRORE CALCOLO INTERVENTO 316 MISURA 19, NESSUN CAPO HA SUPERATO IL CONTROLLO PER IL PREMIO");
				...
				logger.error("ERRORE CALCOLO INTERVENTO 316 MISURA 19, ERRORE DURANTE L'ESECUZIONE DEL CALCOLO INTERVENTO 316 MISURA 19 REF02.008");
				new Dmt_t_errore(getSessione(), "ref02_008", getInput(), e.getMessage());
			} catch (NullPointerException e) {
				logger.error("ERRORE DURANTE L'ESECUZIONE DEL CALCOLO INTERVENTO 316 MISURA 19, NESSUN CAPO DISPONIBILE");
		...
		if(logger.isDebugEnabled())
			logger.debug("CALCOLO INTERVENTO 316 MISURA 19, FINE ESECUZIONE");
	 ...
	 * {@link Dmt_t_output_esclusi}. Dei capi non ammessi a premio sarà salvata
	 * l'informazione di identificazione del capo, il premio per cui è stata
	 * effettuata la richiesta di amissione e la motivazione per cui risulta non
	 * idoneo al premio. Per i capi risultanti idonei al premio in questione,
	 * sarà salvata l'informazione dell'anno campagna per cui concorrono, il
	 * numero di capi ammessi a premio, il cuaa che ha presentato la domanda e
	 * il codice premio.
	 * 
	 * @throws ControlloException
	 *             eccezione relativa al controllo di tipo
	...
	public void postEsecuzione() throws ControlloException {
		...
		if(logger.isDebugEnabled())
			logger.debug("CALCOLO INTERVENTO 316 MISURA 19, INIZIO POST-ESECUZIONE");
		...
		if(logger.isDebugEnabled())
			logger.debug("CALCOLO INTERVENTO 316 MISURA 19, FINE POST-ESECUZIONE");
		 logger.info("FINE ESECUZIONE CALCOLO INTERVENTO 316 MISURA 19");
	}
